    ...
    ContractPostApprovalService postApproval) : ControllerBase
{
    ...
    public async Task<IActionResult> PendingCount(CancellationToken ct)
    {
        ...
        var count = await CountOpenActionsForUserAsync(userId, ct);
        return Ok(new { count });
    }
        ...
        CancellationToken ct = default)
    {
        ...
        var linkedIds = canReadAll
            ? []
            : await LoadLinkedContractIdsAsync(userId, ct);

        var query = PostApprovalQueryFilters.ApprovedWithPostApproval(
            db.Contracts.AsNoTracking(),
            onlyCompleted);

        if (!canReadAll)
            query = PostApprovalQueryFilters.ForAssignee(query, userId, linkedIds);

        query = PostApprovalQueryFilters.PreFilterStatus(query, activeView, statusFilter);
        query = PostApprovalQueryFilters.PreFilterSearch(query, q);

        var rows = await query
            .OrderByDescending(c => c.CreatedAtUtc)
            .Take(PostApprovalQueryFilters.ListMaxRows)
            .Select(c => new ActionContractRow(
                c.Id,
                c.ContractNumber,
                c.Title,
                c.SubjectPersonName,
        ...
        {
            var state = PostApprovalJsonHelper.DeserializeState(row.PostApprovalJson);
            ...
            if (!string.IsNullOrWhiteSpace(statusFilter)
                && !string.Equals(state.Status, statusFilter, StringComparison.OrdinalIgnoreCase))
                continue;

            var sortAt = state.UpdatedAtUtc ?? state.CompletedAtUtc ?? row.CreatedAtUtc;
            if (fromUtc.HasValue && sortAt < fromUtc.Value)
                continue;
            if (toUtc.HasValue && sortAt > toUtc.Value)
                continue;

            if (!string.IsNullOrWhiteSpace(q))
            {
                var term = q.Trim();
                if (!row.ContractNumber.Contains(term, StringComparison.OrdinalIgnoreCase)
                    && !row.Title.Contains(term, StringComparison.OrdinalIgnoreCase)
                    && !row.SubjectPersonName.Contains(term, StringComparison.OrdinalIgnoreCase)
    ...
    private async Task<int> CountOpenActionsForUserAsync(Guid userId, CancellationToken ct)
    {
        var linkedIds = await LoadLinkedContractIdsAsync(userId, ct);

        var query = PostApprovalQueryFilters.ApprovedWithPostApproval(
            db.Contracts.AsNoTracking(),
            archived: false);
        query = PostApprovalQueryFilters.ForAssignee(query, userId, linkedIds);
        query = PostApprovalQueryFilters.PreFilterStatus(query, activeView: true, status: null);

        var jsonRows = await query
            .Select(c => c.PostApprovalJson)
            .ToListAsync(ct);

        var count = 0;
        foreach (var json in jsonRows)
        {
            var state = PostApprovalJsonHelper.DeserializeState(json);
            if (state is null || state.AssigneeUserIds.Count == 0)
                continue;
            if (!state.AssigneeUserIds.Contains(userId))
                continue;
    ...
    private Task<List<Guid>> LoadLinkedContractIdsAsync(Guid userId, CancellationToken ct) =>
        db.ContractActionLinks.AsNoTracking()
    ...
    private async Task EnrichStepNamesAsync(List<PorslineClone.Application.Contracts.ApprovalStepDto> steps, CancellationToken ct)
    {
        ...
        {
            if (!map.TryGetValue(s.UserId, out var u))
                continue;
    ...
    private sealed record ActionContractRow(
        Guid Id,
        string ContractNumber,
        string Title,
        string SubjectPersonName,
        string PostApprovalJson,
